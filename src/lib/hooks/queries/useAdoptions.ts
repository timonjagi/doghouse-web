...
export const useAdoptionTimelineLogic = ({
  ...
  const hasPendingReservationPayment = pendingTransactions.some(
    (tx) => (tx.meta as any)?.payment_type === "reservation"
  );
  const hasPendingFinalPayment = pendingTransactions.some(
    (tx) => (tx.meta as any)?.payment_type === "final"
  );
    ...
    if (isApplicant) {
      ...
      const steps: TimelineStep[] = [
        {
          ...
          description:
            "Your adoption application has been received and is awaiting review",
          status: "completed",
          date:
            getStatusDate("submitted") ||
        ...
        steps.push({
          ...
          title:
            adoption.status === "approved"
              ...
              : "Decision Made",
          description:
            adoption.status === "approved" || adoption.status === "reserved"
          ...
          status: ["approved", "rejected", "reserved"].includes(adoption.status)
            ? "completed"
      ...
      if (["approved", "reserved"].includes(adoption.status)) {
        steps.push({
          ...
          description: adoption.reservation_paid
            ? "The listing has been reserved for you"
            : "Pay reservation fee to secure your adoption",
      ...
      if (
        ...
        (adoption.reservation_paid && !contractRequired)
      ) {
        steps.push({
          id: "payment",
      ...
      if (adoption.payment_completed) {
        steps.push({
          ...
          description:
            adoption.status === "completed"
              ? "Congratulations! Adoption process is complete. If you enjoyed your experiece, please leave us a review."
              : "Contact the breeder for pickup/delivery arrangements",
        ...
        },
        {
          ...
          title:
            adoption.status === "approved"
              ...
              ? "Adoption Rejected"
              : "Adoption Approved",
          description: ["approved", "reserved", "completed"].includes(
            adoption.status
          )
          ...
          status: ["approved", "rejected", "reserved", "completed"].includes(
            adoption.status
          )
            ? "completed"
            : adoption.status === "pending"
      ...
      if (["approved", "reserved", "completed"].includes(adoption.status)) {
        steps.push({
          ...
          description: adoption.reservation_paid
            ? "Reservation fee has been paid"
            : "Waiting for applicant to pay reservation fee",
          status: adoption.reservation_paid ? "completed" : "current",
          date: getStatusDate("reserved"),
          info: [
      ...
      if (adoption.reservation_paid && contractRequired) {
        steps.push({
          ...
          description: adoption.contract_signed
            ? "Contract has been signed"
            : "Waiting for applicant to sign adoption contract",
          status: adoption.contract_signed
            ? "completed"
            : adoption.reservation_paid
            ? "current"
      ...
      if (
        ...
        steps.push({
          ...
          description: adoption.payment_completed
            ? "Final payment has been completed"
            : "Waiting for applicant to complete final payment",
          status: adoption.payment_completed
            ? "completed"
            : adoption.contract_signed ||
              (adoption.reservation_paid && !contractRequired)
      ...
      if (adoption.payment_completed) {
        steps.push({
          ...
          description:
            adoption.status === "completed"
              ? "Congratulations! Your adoption is complete. If you enjoyed your experiece, please leave us a review."
  ...
  const getCurrentStepIndex = () => {
    ...
    return idx === -1
      ? steps.findIndex((step) => step.status === "locked")
      : idx;
  };
...
export const ADOPTION_ACTION_CONFIGS = {
  ...
  complete: {
    ...
    dialogBody:
      "Marking this adoption as completed will move it to the completed tab and request a review from the applicant.",
  ...
  pay_reservation: {
    ...
    dialogBody:
      "You are about to pay the reservation fee. This will secure the listing for you.",
  ...
  sign_contract: {
    ...
    title: "Sign Contract",
    dialogBody:
      "You are about to sign the adoption contract. Please review the terms carefully.",
...
export const getAvailableAdoptionActions = ({
  ...
  if (isApplicant) {
    ...
    if (
      ["approved", "reserved"].includes(adoption.status) &&
      ...
      buttons.push({
        ...
        variant: "outline",
        payload: {
          reference: (transaction?.meta as any)?.paystack_reference,
          type: "reservation",
        },
      });
    }
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<AdoptionWithListing | null> => {
      ...
      if (data) {
        ...
        if (
          adoptionData.users?.seeker_profiles &&
          Array.isArray(adoptionData.users.seeker_profiles)
        ) {
          adoptionData.users.seeker_profiles =
            adoptionData.users.seeker_profiles[0] || null;
        }

        if (
          adoptionData.listings?.users?.breeder_profiles &&
          Array.isArray(adoptionData.listings.users.breeder_profiles)
        ) {
          adoptionData.listings.users.breeder_profiles =
            adoptionData.listings.users.breeder_profiles[0] || null;
        }
  ...
  return useMutation({
    ...
    onSuccess: async (data) => {
      ...
      try {
        ...
        console.error(
          "Failed to send adoption application notifications:",
          notificationError
        );
      }
      ...
      queryClient.invalidateQueries({
        queryKey: queryKeys.adoptions.received(),
      });
    },
  });
};
