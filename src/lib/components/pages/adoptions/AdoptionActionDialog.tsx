...
import { useUpdateAdoption, AdoptionWithListing, useAdoptionTimelineLogic } from '../../../hooks/queries/useAdoptions';
import {
  AlertDialog,
  AlertDialogOverlay,
  AlertDialogContent,
...
const AdoptionActionDialog: React.FC<AdoptionActionDialogProps> = ({
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    switch (pendingAction.type) {
      ...
      case 'complete_payment':
        try {
          ...
          const result = await initiatePaymentMutation.mutateAsync({
            amount,
            type: pendingAction.type === 'pay_reservation' ? 'reservation' : 'final',
      ...
      case 'contact_applicant':
        try {
          const participants = pendingAction.type === 'contact_breeder'
      ...
      case 'contact_support':
        toast({
          ...
          description: 'Support contact flow is being implemented.',
          status: 'info',
          duration: 3000,
        });
        handleClose();
    ...
    if (['withdraw', 'approve', 'reject', 'complete'].includes(pendingAction.type)) {
      try {
        await updateAdoptionMutation.mutateAsync({
          id: adoption.id,
          updates: {
  ...
  return (
    <AlertDialog isCentered isOpen={isOpen} leastDestructiveRef={undefined} onClose={handleClose} size={isPaymentAction ? "lg" : "md"}>
      <AlertDialogOverlay>
        <AlertDialogContent>
          ...
          <AlertDialogBody>
            <VStack spacing={4} align="stretch">
              ...
              {(isContactAction || ['approve', 'reject', 'complete'].includes(pendingAction?.type)) && (
                <FormControl>
                  <FormLabel fontSize="sm" fontWeight="medium">
                    {isContactAction ? 'Initial Message' : 'Response Message (Optional)'}
                  </FormLabel>
                  <Textarea
                    value={responseMessage}
                    onChange={(e) => setResponseMessage(e.target.value)}
                    placeholder={
                      isContactAction ? "Type your message here..." :
          ...
          <AlertDialogFooter borderTopWidth={isPaymentAction ? "1px" : 0} mt={2}>
            <Button isDisabled={isLoading} onClick={handleClose} size="sm" variant="ghost">
              Cancel
            </Button>
            <Button
              ...
              isLoading={isLoading}
              size="sm"
              loadingText={isPaymentAction ? "Initializing..." : "Submitting..."}
            >
              {isPaymentAction ? `Pay Ksh. ${amount.toLocaleString()}` : (pendingAction?.confirmText || 'Confirm')}
            </Button>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialogOverlay>
