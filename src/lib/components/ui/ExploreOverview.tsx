...
const ExploreOverview: React.FC = () => {
  ...
  } = useSeekerDashboard(filters.category);
  ...
  const getTabIndex = (tabParam: string | string[] | undefined): number => {
    const index = categories?.tabMenuItems.findIndex(
      (item: Category) => item.tab === tabParam
    );
  ...
  return (
    <Box w="full" minH={{ base: "auto", md: "5s00px" }} bg="bg-surface">
      <TabBar
        menuItems={categories?.tabMenuItems || []}
        activeTab={activeTab}
      />
      ...
      >
        ...
        <Stack bg={{ base: "", md: mode("white", "gray.800") }}>
          <TabPanels>
            <TabPanel p={0}>
              <Stack spacing={6}>
                <Flex flex="1" fontSize="sm" overflow="auto">
                  <NavCategoryMenu.Mobile menus={categories.petTypes} />
