...
const DesktopNavMenu = ({ data }: DesktopNavMenuProps) => {
  return (
    <Box px="10" pt="6" pb="16" bg={mode("white", "gray.800")}>
...
const MobileNavMenu = ({ data }: MobileNavMenuProps) => (
  <Accordion allowToggle defaultIndex={0} overflow="auto" w="full">
    <Stack width="full">
      {data.products.map((product) => (
        <NavAccordionItem label={[product.label]} width="full">
          <HStack width="full" align="baseline" justify="space-between">
            ...
            <HStack>
              <Link
                href={`/dashboard/search?tab=listings&category=${product.label.toLowerCase()}`}
              >
                View All
              </Link>
              <ChevronRightIcon />
          ...
          <Stack direction="row" pt="5" width="full" spacing="6">
